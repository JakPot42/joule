...
builder, real, and ties directly into the NUWC cluster (P18 Undersea
Acoustic Intelligence Suite, P19 Cable Resilience Analyzer). Naval Station
...
center, and — via the GridPulse (P31) bridge — a real HIGH grid-stress
...
on it" discipline the rest of this portfolio has hit repeatedly (P38's
CMS pricing substitution, P44's FAA data check, P59's missing spec) —
...
## GridPulse (P31) integration

`gridpulse_bridge.py` reads GridPulse's real `to_joule_format()` export —
loose file-based coupling (separate repos, "aware of each other" per
CLAUDE.md), not a cross-repo Python import. `data/gridpulse_export_sample.json`
